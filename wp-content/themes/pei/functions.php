...
function pei_register_component_blocks() {
    ...
    foreach ( glob( $components_dir . '/*', GLOB_ONLYDIR ) as $component_path ) {
        ...
        if ( file_exists( $editor_script ) ) {
            ...
            wp_register_script(
                $editor_handle,
                $component_uri . $component . '-editor.js',
                array(
                    'wp-blocks',
                    'wp-element',
                    'wp-components',
                    'wp-i18n',
                    'wp-block-editor',
                    'wp-editor',            // ensure RichText is available in more setups
                    'wp-server-side-render',
                    'wp-rich-text',         // helper library, safe to include
                ),
                PEI_VERSION,
                true
            );
